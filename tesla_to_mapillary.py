    ...
    for filename in os.listdir(basepath):
        if filename.endswith(camera + ".mp4"):
            targetpath = os.path.join(
                basepath, camera, rchop(filename, "-" + camera + ".mp4")
            )
            if not os.path.isdir(targetpath):
                os.mkdir(targetpath)
                os.rename(
                    os.path.join(basepath, filename), os.path.join(targetpath, filename)
                )
...
def process_camera_dir(basepath, camera, gpx_file, mapillary_username, localtz):
    ...
    subfolders = [
        f.path for f in os.scandir(os.path.join(basepath, camera)) if f.is_dir()
    ]

    for path in subfolders:
        for filename in os.listdir(path):
            if filename.endswith(camera + ".mp4"):
                tesla_datetime = datetime.strptime(
                    filename[0:19], "%Y-%m-%d_%H-%M-%S"
                )  # Pattern: 2020-09-24_14-06-34-front.mp4
                ...
                subprocess.run(
                    [
                        ...
                        "0.5",
                        "--video_start_time",
                        tesla_datetime.strftime("%Y_%m_%d_%H_%M_%S_%f"),
                        "--geotag_source",
                        "gpx",
                        "--geotag_source_path",
                        gpx_file,
...
if __name__ == "__main__":
    my_parser = argparse.ArgumentParser(
        description="Process Tesla dashcam videos with accompanying GPX file"
    )
    my_parser.add_argument(
        "videopath", metavar="videopath", type=str, help="The videopath to list"
    )
    my_parser.add_argument(
        "gpxfilepath", metavar="gpxpath", type=str, help="The gpx file"
    )
    my_parser.add_argument(
        "mapillary_user", metavar="mapillary_user", type=str, help="Mapillary username"
    )
    my_parser.add_argument(
        "timezone",
        metavar="timezone",
        ...
        help="Timezone the Tesla ride was in. Default: Europe/Zurich",
    )
